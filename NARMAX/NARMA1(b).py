...
class NARMA:
    ...
    def forward(self, x_input, u_input):
        a_hidden = np.dot(self.weights_input_hidden, np.concatenate((x_input, u_input))) + self.bias_hidden
        z_hidden = self.tanh(a_hidden)
        output = np.dot(self.weights_hidden_output, z_hidden) + self.bias_output
    ...
    def train(self, x, n_epochs = 100):
        ...
        best_weights_hidden_output = self.weights_hidden_output
        best_bias_output           = self.bias_output
        best_weights_input_hidden  = self.weights_input_hidden
        best_bias_hidden           = self.bias_hidden
        best_loss = np.inf
        for epoch in range(n_epochs):
            total_loss = 0.0
            for x_input, y_true in zip(X, Y):
                u_input = np.random.normal(scale=0.1, size=self.n_u)
                y_pred, z_hidden = self.forward(x_input, u_input)
                error = y_pred - y_true
                ...
                dL_dweights_hidden_output = dL_dy_pred   * z_hidden
                dL_dbias_output           = dL_dy_pred
                dL_dz_hidden              = dL_dy_pred   * self.weights_hidden_output
                dL_da_hidden              = dL_dz_hidden * self.tanh_derivative(z_hidden)
        ...
        # return the best weights and biases
    ...
    def print_model(self):
        ...
        print(f"n_u: {self.n_u}\n")
        print(f"Weights Input to Hidden: {self.weights_input_hidden.tolist()}\n")
        print(f"Biases Hidden: {self.bias_hidden.tolist()}\n")
        print(f"Weights Hidden to Output: {self.weights_hidden_output.tolist()}\n")
        print(f"Bias Output: {self.bias_output}\n")
...
##    compare = narma_loaded.weights_input_hidden==narma.weights_input_hidden
##    same = reduce(operator.and_, compare, True)
##    same = reduce(operator.and_, same, True)
##    print(f"Is network saved equal to netwokk loaded?: {same}")
